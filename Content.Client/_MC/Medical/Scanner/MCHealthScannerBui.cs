...
[UsedImplicitly]
public sealed class MCHealthScannerBui : BoundUserInterface
{
    ...
    [ViewVariables]
    private MCHealthScannerWindow? _window;
    ...
    public MCHealthScannerBui(EntityUid owner, Enum uiKey) : base(owner, uiKey)
    {
    ...
    private void UpdateState(HealthScannerBuiState uiState)
    {
        if (_window == null)
        {
            _window = this.CreateWindow<MCHealthScannerWindow>();
    ...
    private void MedicalAdvice(Entity<DamageableComponent> target, HealthScannerBuiState uiState, MCHealthScannerWindow window)
    {
    ...
    private void AddAdvice(string text, MCHealthScannerWindow window)
    {
        var label = new RichTextLabel();
        label.SetMarkupPermissive(text);
